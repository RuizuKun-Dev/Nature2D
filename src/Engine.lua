...
function Engine:Create(object, properties)
    ...
    for prop, value in pairs(properties)do
        ...
        if Globals.OBJECT_PROPS_TYPES[prop] and typeof(value) ~= Globals.OBJECT_PROPS_TYPES[prop] then
            error(string.format(
[[[Nature2D]: Invalid Property type for %q. Expected %q got %q.]], prop, Globals.OBJECT_PROPS_TYPES[prop], typeof(value)), 2)
        end
    end
        ...
        if properties.Structure then
            ...
            for _, c in ipairs(properties.Structure)do
                ...
                if typeof(a) ~= 'Vector2' or typeof(b) ~= 'Vector2' then
                    error(
[[[Nature2D]: Invalid point positions for custom RigidBody structure.]], 2)
                end
        ...
        local newBody = RigidBody.new(obj, properties.Mass or self.mass, properties.Collidable, properties.Anchored, self, properties.Structure and custom or nil, properties.Structure)
